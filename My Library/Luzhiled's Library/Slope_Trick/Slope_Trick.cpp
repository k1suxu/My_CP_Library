...
//explanation
/*
query() : min(f(x))Ç∆ÇªÇÃéûÇÃxÇÃç≈è¨ílç≈ëÂílÇï‘Ç∑
add_all(a) : f(x) = f(x) + a
add_a_minus_x(a) : f(x) = f(x) + max(a-x, 0) -> âEå®â∫Ç™ÇËÇ©ÇÁíºê¸
add_x_minus_a(a) : f(x) = f(x) + max(x-a, 0) -> íºê¸Ç©ÇÁâEå®è„Ç™ÇË
add_abs(a) : f(x) = f(x) + |x-a| = f(x) + max(a-x, 0) + max(x-a, 0)
clear_right() : f(x) = 
clear_left() : 
shift(a, b) : f(x)=min_{x-b<=y<=x-a}f(y) (a<=bÇ™ïKê{)
shift(a) : f(x) = f(x-a)
get(x) : f(x)Çï‘Ç∑(fÇÕîjâÛÇ≥ÇÍÇÈ)
merge(g(x)) : f(x) = f(x) + g(x) (gÇÕîjâÛÇ≥ÇÍÇÈ)
*/