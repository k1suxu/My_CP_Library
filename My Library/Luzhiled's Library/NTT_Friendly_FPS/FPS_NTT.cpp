...
struct NumberTheoreticTransformFriendlyModInt {
  ...
  static void init() {
    if(roots.empty()) {
      const unsigned mod = Mint(0).get_mod();
      assert(mod >= 3 && mod % 2 == 1);
      auto tmp = mod - 1;
      max_base = 0;
      while(tmp % 2 == 0) tmp >>= 1, max_base++;
      Mint root = 2;
      while((root^((mod - 1) >> 1)) == 1) {
        root += 1;
      }
      assert((root^(mod - 1)) == 1);
      ...
      roots[max_base] = (root^((mod - 1) >> max_base));
...
struct FormalPowerSeriesFriendlyNTT : vector< T > {
  ...
  //not-verified because my mint doesn't have the sqrt()
  ...
  P pow(int64_t k, int deg = -1) const {
    ...
    for(int i = 0; i < n; i++) {
      if((*this)[i] != T(0)) {
        T rev = T(1) / (*this)[i];
        P ret = (((*this * rev) >> i).log() * k).exp() * ((*this)[i]^k);
